...
The matched-channel interface now derives those chart-channel limits from the
exact recovered CSpec residuals themselves: summed recovered count, curvature,
and spectral/locality residuals tend to zero, and matched scalar chart
certificates therefore instantiate the physical-chart bridge without separate
convergence assumptions.
The scheduled-density interface now removes the remaining density-at-infinity
input when the physical chart certificates obey a positive affine refinement
law for sprinkling density.  Lean proves that such a schedule tends to
`atTop`, then reuses the matched physical-chart bridge unchanged.
The operator-split interface factors the remaining 4D BDG analytic package into
