#!/usr/bin/env python3
"""LAMBDA-OBSERVABLE HARDWARE RUN v4: CROSSTALK DISCRIMINATOR.
...
def circuit_line(d):
    """v4: as v3, but omitted records are replaced by DOUBLE CNOTs
    (logical identity, same locality) - shallow circuits now carry
    MORE neighboring 2q activity than deep ones.  If the record-0
    excess of v3 is spectator crosstalk it must collapse or flip
    sign here; if it is record physics it persists."""
...
counts2q = [c.count_ops().get("cz", 0) + c.count_ops().get("ecr", 0)
            for c in isa]
log(f"transpiled 2q-gate counts (should FALL by 1 per depth, v4): {counts2q}")
...
log("--- VERDICT (registered readings, v4 discriminator) ---")
...
meta = {
    ...
    "protocol": "v4-double-cnot-filler",
...
out = (f"logs_lambda_hw_v4_{backend.name}_"
