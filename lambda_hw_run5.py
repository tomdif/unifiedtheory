#!/usr/bin/env python3
"""LAMBDA-OBSERVABLE HARDWARE RUN v5: READOUT-MATCHED LINE-WALK.
...
V5 CHANGE (readout matching): v3/v4 replicated a +9.7 sigma excess
isolated to record 0, declining ~1%/depth even when added gates are
3+ qubits away - but deeper circuits MEASURE more qubits
simultaneously, and readout crosstalk depolarizing toward 0.5 with
strength ~ (P - 1/2) predicts the record-1 effect ~6x smaller than
record-0, as observed.  v5 measures ALL T ancillas in EVERY circuit
(identical simultaneous-readout structure; unwritten ancillas are
measured and ignored).  If the record-0 excess vanishes, the cause
was measurement crosstalk and the null bound stands; if it
persists, escalate.
...
log("--- VERDICT (registered readings, v5 readout-matched) ---")
...
out = (f"logs_lambda_hw_v5_{backend.name}_"
