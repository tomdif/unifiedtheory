...
## Profile-Sequence Bridge

Integrated in:

`UnifiedTheory/Audit/KFCausalCSpecRecoveredStageBDGProfile.lean`

The file defines:

```lean
structure BDGProfileSequenceAsymptotics
...
```lean
theorem BDGProfileSequenceAsymptotics.layer_asymptotics

theorem RecoveredStageBDGProfileSequenceInterface.profile_bdg_dalembertian_tendsto

theorem RecoveredStageBDGProfileSequenceInterface.rssPoissonError_zero_and_profile_bdg_dalembertian_tendsto

theorem RecoveredStageBDGProfileSequenceInterface.rssPoissonError_zero_and_standard_profile_bdg_dalembertian_tendsto
```

This closes the parameter-shape mismatch between the 4D BDG gate/profile
theorems, which are stated for a real high-density parameter, and the recovered
CSpec refinement sequence indexed by `n : Nat`.  If `density n -> atTop` and
