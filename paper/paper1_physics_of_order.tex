...
25 core files in the derivation chain ($\sim$4{,}600 lines), within a
...
computes nine zero-parameter predictions (five mass ratios, the
Cabibbo angle, the CKM hierarchy, the Higgs mass-to-VEV ratio, and
the electroweak scale) from the algebraic structure derived here.
...
(14 theorems): every linear order is both future- and
...
\noindent\textit{Lean verification: 25 core files, $\sim$4{,}600 lines
...
\texttt{Asymptotic\-Freedom.lean} (15 theorems).  For the
...
Discrete Ambrose-Singer & Lean~4 & Zero sorry (27 thms) \\
Casimir scaling ($m_t/m_b$) & Lean~4 & Zero sorry (9 thms) \\
Asymptotic freedom & Lean~4 & Zero sorry (15 thms) \\
Distinguishing spacetime & Lean~4 & Zero sorry (14 thms) \\
Neutrino scale $M_R = M_P$ & Lean~4 & Zero sorry (8 thms) \\
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{The Born rule is the unique SO(2)-invariant
    quadratic form.}  The K/P split yields a natural SO(2) rotation
    symmetry between the position-like (K) and momentum-like (P)
    components.  The observable $Q^2 + P^2$ is the unique
    rotation-invariant quadratic form.  Linearity is a necessary
    condition from counting (Section~2); the probabilistic
    interpretation follows from the SO(2) invariance (Paper~II).
  ...
  \item \textbf{Dark matter is a mandatory prediction.}
    ...
    the K/P structure, not an optional addition (Paper~III).
  ...
  \item \textbf{The seesaw scale is fixed.}  The partial order has
    ...
    $m_1 \sim v^2/M_P \approx 0.005$~eV --- a testable prediction
    within reach of Project~8 and KATRIN successor experiments.
  ...
  \item \textbf{There is no landscape of vacua.}  Every discrete
    choice --- gauge group, matter content, generations, hypercharges
    --- is uniquely forced.  The only continuous parameter is the
    sprinkling density $\rho$, a single real number.  There is no
    analog of the string landscape.
...
The complete Lean~4 source code is available at
\url{https://github.com/tomdif/unifiedtheory} (tag v2.0.0).
Build instructions: \texttt{lake build} (2{,}367 jobs, zero errors).
