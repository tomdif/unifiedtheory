...
\begin{abstract}
We present a formally verified parent-structure framework in which gravity, gauge structure, a defect-based matter sector, and a quantum-amplitude layer arise from a common formal architecture. The algebraic development proves renormalization rigidity ($\alpha = d{-}1$ from spatial dimension), null-cone determination, causal counting and null-link relations, source/dressing decomposition from a source functional, Riemann and Bianchi identities from metric derivative symmetries, the divergence-free Einstein tensor, complete 4D Lovelock uniqueness within the tensorial second-order natural class (including contraction classification, Gauss--Bonnet vanishing, and $\varepsilon$-tensor exclusion), full nonabelian Yang--Mills structure including the nonabelian Bianchi identity, linear defect charge algebra with multiple conserved charges, spin-statistics structure, representation covariance under the adjoint action, and uniqueness results for the complex amplitude and Born-type quadratic observable within the stated symmetry class. A dynamical decoherence layer is provided by density-matrix and Lindblad-type dephasing models with a proven classical limit. Dimension selection proves $d = 3$ is the unique spatial dimension with both stable orbits and clean wave propagation. Each of five framework primitives is proven necessary (removing any gives a degenerate theory). A discrete causal-counting model suggests a scaling relation $\Lambda \sim N^{-1/2}$ for the cosmological constant.

Separate deterministic computational models reproduce nontrivial gravitational observables in $3{+}1$ dimensions and canonical quantum benchmarks including double-slit interference, Aharonov--Bohm phase shift, Tsirelson-saturating CHSH violation, Mach--Zehnder erasure, and GHZ/Mermin scaling. The algebraic core is formally verified in Lean~4 with zero custom axioms and zero \texttt{sorry} statements. The result is a formally closed candidate framework with explicit primitive assumptions, together with concrete realizations, rather than a claim of final closure of fundamental physics.
\end{abstract}
...
Each of the five framework primitives (source functional, linear composition, Lorentzian signature, Lie algebra, perturbation dimension $\geq 2$) is proven \emph{necessary}: removing any one gives a provably degenerate or trivial theory.
