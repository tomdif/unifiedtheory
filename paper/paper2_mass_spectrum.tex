...
\title{Fermion Mass Ratios from the K/P Projection:\\
  Nine Predictions from One Parameter, Including the Higgs Mass and Electroweak Scale\\[6pt]
...
observables from the K/P projection mechanism.  The sprinkling
density $\rho$ (which sets the Planck scale) is the single free
parameter; mass ratios are density-independent. The source functional $\phi$ induces a split into K
...
relation $|V_{us}| \approx \sqrt{m_d/m_s}$ (an independent ansatz,
not derived from the framework), applied to the derived
...
the multiplicative factorization of these corrections is a modeling
choice.
...
Nine predictions from a single free parameter (the sprinkling
density $\rho$, which sets the Planck scale), spanning seventeen
orders of magnitude, are reported; eight of nine are within a
...
$\rho$.  The algebraic core (gauge group, charges, dimension) has
no free parameters; the only continuous parameter is $\rho$, which
sets the Planck scale.  Mass ratios are density-independent
(Section~5).
...
and $a = c$, then extends to all $\theta$.  The quadraticity itself
is derived from additivity of probabilities and composition of
subsystems (\texttt{BornRuleQuadratic.lean}): no other polynomial
...
\texttt{BornRule\-Quadratic.observable\_must\_be\_degree\_2}.
...
The primary prediction is $m_\mu/m_\tau = 0.056 \pm
...
therefore reported as a consistency check, not as an independent
prediction. The count-weighted result ($0.056$ at $\rho = 10{,}000$) is
the primary prediction; $\rho$ is the single free parameter.
...
to within $6\%$ (factor $0.94\times$).  Variation across densities
is consistent with seed-to-seed fluctuations, not a systematic trend
(see density-independence analysis below).
...
with $\rho$ as the single free parameter.

\noindent\textbf{The mass ratio is density-independent.}
At $\rho = 2$K, $5$K, and $10$K, the fitted ratio is $0.060$,
$0.072$, and $0.061$ respectively, with variation consistent with
seed-to-seed fluctuations (no systematic trend).  The decoherence
contributes a multiplicative suppression that affects both
components equally and cancels in the ratio:
$|c_{\mathrm{light}}|/|c_{\mathrm{heavy}}|
= (A_{\mathrm{light}} e^{-\Gamma L}) / (A_{\mathrm{heavy}} e^{-\Gamma L})
= A_{\mathrm{light}}/A_{\mathrm{heavy}}$.
The sprinkling density $\rho$ sets the overall energy scale but
does not enter the mass ratios.  The mean across densities,
$r_{\mu\tau} = 0.062$, is within 4\% of experiment ($0.0595$).
...
(Sections~3--4) propagated through established flavor physics.
The Fritzsch texture is an independent ansatz~[7], not derived
from the K/P framework; the framework's contribution is the mass
ratios that serve as inputs.  No
...
(experiment: $125/246 = 0.509$, factor $0.65$--$0.82\times$) is
determined entirely by the SU(2) holonomy
...
% ============================================================
% 8. CHAIN SAMPLING: A UNIFIED DIAGNOSTIC
% ============================================================
...
% ============================================================
\section{Complete Scorecard: Nine Predictions from One Parameter}
...
confirming that $\beta = 4$ requires no correction.  A three-density
scan ($\rho = 2$K, $5$K, $10$K) confirms the mass ratio itself is
density-independent (mean $0.062$, within $4\%$ of experiment),
with variation consistent with seed-to-seed fluctuations.
...
(Section~9).
These nine
predictions use a single free parameter ($\rho$, the sprinkling
density, which sets the Planck scale): the gauge group, coupling
constants, generation count, and generation phase $\alpha =
\alpha_{\mathrm{em}}(M_P)$ are all derived in Paper~I.  The mass
ratios are density-independent (the decoherence contribution
cancels in the ratio); $\rho$ enters only through the absolute
energy scale.
...
(measured at $\rho = 2$K, $5$K, $10$K), confirming that the SU(2)
coupling requires no density-dependent correction.  The mass ratio
is also density-independent: fitted values $0.060$, $0.072$, $0.061$
at $\rho = 2$K, $5$K, $10$K respectively, with no systematic trend.
The decoherence suppression is multiplicative and cancels in the
ratio (Section~5).

% ============================================================
% 10. THE m_t/m_b RATIO
% ============================================================
...
\noindent The multiplicative factorization of the weak SU(2)
projection, the color Casimir ratio, and the hypercharge ratio is
an assumption about how the color and electroweak sectors compose
in the K/P correlator.  Each individual factor is a theorem
(formally verified in Lean~4, \texttt{CasimirScaling.lean}),
but the factorization itself is a modeling choice:
\begin{itemize}[nosep]
...
Nine predictions from a single free parameter ($\rho$, which sets
the Planck scale) across quarks, leptons, CKM mixing,
...
zero \texttt{sorry}, zero custom axioms) and GPU Monte Carlo scripts
are available at \url{https://github.com/tomdif/unifiedtheory}.
All numerical results reported in this paper are
reproducible from the scripts in the \texttt{scripts/} directory.
The script \texttt{decoherence\_separation.py} tests the hypothesis
that measured mass ratios contain a density-dependent decoherence
contribution.  Result: the mass ratio is density-independent
($R^2 = 0.007$), confirming that decoherence cancels in the ratio
$|c_{\mathrm{light}}|/|c_{\mathrm{heavy}}|$ as a common
