...
Gate 6 now has a finite/audit specialization:
...
unrelated parents at successive ranks. This finite-law module itself does not
choose a Gate 3 observation map.

`KFCausalSetCompleteChiralBornTrajectoryMeasure.lean` now closes the distinct
infinite probability-extension step. The stagewise Born probabilities define
history-dependent Markov kernels; Ionescu--Tulcea produces a probability
measure on complete rank-dependent causal-growth trajectories; and the regular
conditional distribution of the next branch given the full finite history is
exactly the complete-chiral Born kernel. Positive conditional singleton mass is
equivalent to a physical one-element extension. This removes the infinite-path
measure from the open list, but does not provide a continuum spacetime limit,
the fixed-cell Gate 3 observation map, or the stoppable repair inequalities.
`KFCausalSetCompleteChiralBornTrajectoryExtension.lean` additionally proves the
exact finite singleton-cylinder formula and that the measurable set of
all-depth physical trajectories has measure one.

`KFCausalSetCompleteChiralBornChiralityBlindness.lean` exposes a further exact
consequence of that construction. The two reflected complete-chiral amplitude
laws are pointwise complex conjugates, hence have equal squared moduli, equal
stage normalizers, equal normalized PMFs, equal history kernels, and ultimately
the same Ionescu--Tulcea trajectory measure. This is a useful no-go/identifiability
boundary: no statistic of the classical causal-growth path can select the
chirality label. A successful chiral phenomenology must retain coherent phase
or interference data, or introduce a chirality-sensitive interaction before
taking squared moduli.

The action-selection branch now replaces the old fixed-coupling witness by the
vacuum-action-selected harmonic running schedule.
`KFTOEGate1HarmonicBornShellSelection.lean` applies the separately defined
canonical positive-radial Born completion and closes its coherent/Born
normalized, projective, strongly positive, reflection-conjugate law package;
`KFCausalSetHarmonicBornTrajectoryMeasure.lean` gives its exact infinite
trajectory law with almost-sure physical support. The first nontrivial PMF is
exactly `(1/2, 1/2)`. The positive-radial representative's 140-edge atlas
support remains the isolated finite real-radical cancellation target.
`KFCausalSetPhaseSafeHarmonicBornShell.lean` supplies an unconditional
alternative by exploiting the otherwise free phase of the fixed-modulus Born
scale. Finite phase avoidance produces a reflection-paired law with exact
physical support and nonzero atlas amplitude. This is an additional
noncomputable full-support selection principle, not a gauge equivalence or a
derivation of the unique positive-radial law.
...
The complete-chiral covariance adapter has now been audited one level more
sharply. `KFCausalCSpecCompleteChiralRankZeroVarianceNoGo.lean` proves it is
uninhabited as stated: the rank-zero branch type has one element, so every
finite observation produces a Dirac Born distribution and variance is exactly
zero, while the record requires nonzero variance at every stage. This premise
is no longer classified as merely “external”. The viable interface instead
uses `KFCausalCSpecVarianceSafeHorizonProjection.lean`: zero variance under
nonnegative unit weights forces every covariance and the relevant quadratic
leakage to vanish, so the projection is total and requires no root-stage
division.
`KFCausalCSpecCompleteChiralPositiveRankObservation.lean` supplies that shift's
observation data without an external labeling: the gregarious empty-precursor
child is cell `1` and every other child is cell `0`. For every positive parent,
the distinct timid full-precursor child witnesses positive complementary mass;
exact physical Born support then proves both cell weights positive and the
binary horizon variance strictly positive. The observation-map and
nondegenerate-variance entries are therefore closed for `n > 0`.
The viable recovery replacement currently formalized in
`KFCausalCSpecMicroscopicGate3WellFoundedRank.lean` avoids covariance division
entirely. Its finite defect rank is zero exactly at quantized residual vacuum
and canonical bridge transport, and a strict-decrease/absorbing-zero update
reaches permanent operational recovery by stage equal to the initial rank.
The remaining constructive problem is therefore to derive that discrete
rank-step law from a concrete causal-growth update.
`KFCausalCSpecHarmonicBornProtectedWellFoundedGate3.lean` supplies the harmonic
weights and variance-safe protected source and proves finite recovery from
that rank law. It deliberately does not assume that horizon protection alone
causes descent; the source-to-update/rank connection remains physical input.
...
independent targets are the source-driven rank update, Gate 1's displayed-atlas
noncancellation if required, Gate 4's microscopic-to-chart/kernel and horizon
bridges, the Gate 5 continuum/QFT bridges, and the Gate 6 physical
cosmology/evaporation bridges. The harmonic Gate 4 theorem remains explicitly
...
harmonic schedule; its canonical positive-radial Born completion is a separate
construction, and its 140-edge noncancellation check remains explicit in one
chirality while reflection supplies the other. The capstone's Gate 3/4 input
is now the actual harmonic protected-source handoff. The canonical infinite
physical trajectory measure removes Gate
6's bare causal-history measure hypothesis, but not the typed cosmological
readout. The remaining frontier includes the source-driven rank update, Gate
4 continuum/horizon dynamics, Gate 5 constructive QFT, and Gate 6's readout,
Hayden--Preskill/late-structure/GW dynamics.
...
gate6_cosmologyBlackHole_closed_of_finiteAudits
```
...
- `KFGate5ChamberScatteringPremiseRemoval.lean` observes that the finite chamber
  witness is already constructed for every finite causal set and removes that
  redundant caller input. This is a noncomputable set-theoretic chamber
  parametrization, not a derivation of continuum Haag--Ruelle dynamics; the
  continuum spin-statistics lift remains open.
...
including exact finite marginals and almost-sure physical histories. It does
not rename that measure as cosmology: the remaining typed
...
- microscopic black-hole scrambling, decoupling, and Uhlmann-style recovery
  channel dynamics beyond the finite entropy/Page, Hayden-Preskill skeleton,
  AMPS, and finite information-preservation audits.
...
Current viable route (2026-08-25; this supersedes the historical numbered
target inventory below):

1. Derive the harmonic repair `rank_step` from an explicit source-driven causal
   update. The variance-safe source and finite termination theorem are proved,
   but their dynamical connection is not.
...
3. Close Gate 1's displayed 140-edge positive-radial atlas by exact
   real-radical noncancellation, or justify and select a different phase rule.
   The law-level certificate built from the action-selected raw schedule and
   its infinite trajectory measure are already closed.
   `KFCausalSetPhaseSafeHarmonicBornShell.lean` proves that
   a noncomputable reflection-paired full-support phase convention exists and
   closes its own atlas, but that convention changes branch probabilities and
   is not uniquely selected by the vacuum action.
...
5. Define a measurable causal-history-to-cosmology readout and prove its
   physical admissibility, then derive scrambling/decoupling/recovery, late
   structure, and gravitational-wave compatibility. The underlying harmonic
   causal-growth probability measure is already constructed.
...
9. Instantiate
   `microscopicTOEClosureTargetsWithQuantizedGate2FiniteGate5Gate6Audits_closed`.
   The remaining assumptions are Gate 1, the four Gate 4 assumptions, the four
...
5. Gate 5: build the effective Hilbert/QFT limit beyond the now-checked local
