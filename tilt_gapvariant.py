#!/usr/bin/env python3
"""GAP-VARIANT tilt test (selection robustness). Original docstring:: the lognormal-tilt mechanism test.
...
while frontier:
    ...
    for p in frontier:
        ...
        for ck, (mu, g) in children[p].items():
            gg.setdefault(g, []).append((ck, mu))
        gaps = sorted(gg)
        muv = _np.array([sum(m for _, m in gg[g]) for g in gaps], float)
        Av = _np.vstack([muv * _np.cos(_np.array(gaps) * PHI4),
                         muv * _np.sin(_np.array(gaps) * PHI4)])
        bv = _np.array([1.0, 0.0])
        x = born_point(muv, Av, bv, want="maxent")
