...
theorem centeredSource_floor_of_weighted_anti_alignment
    {ι : Type*} [Fintype ι]
    {w source : ℕ → ι → ℝ}
    {rateFloor : ℕ → ℝ}
    {alignment : ℕ → ι → ℝ}
    (hw_nonneg : ∀ n i, 0 ≤ w n i)
    (hweighted_floor :
      ∀ n i, rateFloor n ≤ w n i * alignment n i)
    (halignment :
      ∀ n i, alignment n i ≤ -centeredSource (w n) (source n) i) :
    ∀ n i,
      rateFloor n ≤
        -(w n i * centeredSource (w n) (source n) i) := by
  intro n i
  have hmul :
      w n i * alignment n i ≤
        w n i * (-centeredSource (w n) (source n) i) :=
    mul_le_mul_of_nonneg_left (halignment n i) (hw_nonneg n i)
  calc
    rateFloor n ≤ w n i * alignment n i := hweighted_floor n i
    _ ≤ w n i * (-centeredSource (w n) (source n) i) := hmul
    _ = -(w n i * centeredSource (w n) (source n) i) := by ring

theorem weighted_floor_of_uniform_weight_alignment
    {ι : Type*} [Fintype ι]
    {w : ℕ → ι → ℝ}
    {rateFloor : ℕ → ℝ}
    {alignment : ℕ → ι → ℝ}
    {weightFloor alignmentFloor : ℝ}
    (hweight_nonneg : 0 ≤ weightFloor)
    (halignment_nonneg : 0 ≤ alignmentFloor)
    (hweight_floor : ∀ n i, weightFloor ≤ w n i)
    (halignment_floor : ∀ n i, alignmentFloor ≤ alignment n i)
    (hrate_floor : ∀ n, rateFloor n ≤ weightFloor * alignmentFloor) :
    ∀ n i, rateFloor n ≤ w n i * alignment n i := by
  intro n i
  have hw_nonneg : 0 ≤ w n i :=
    le_trans hweight_nonneg (hweight_floor n i)
  have hprod :
      weightFloor * alignmentFloor ≤ w n i * alignment n i :=
    mul_le_mul (hweight_floor n i) (halignment_floor n i)
      halignment_nonneg hw_nonneg
  exact le_trans (hrate_floor n) hprod

theorem centeredSource_gamma_floor_of_uniform_weighted_alignment
    {ι : Type*} [Fintype ι]
    {w source : ℕ → ι → ℝ}
    {alignment : ℕ → ι → ℝ}
    {gamma weightFloor alignmentFloor : ℝ}
    (hweight_nonneg : 0 ≤ weightFloor)
    (halignment_nonneg : 0 ≤ alignmentFloor)
    (hweight_floor : ∀ n i, weightFloor ≤ w n i)
    (halignment_floor : ∀ n i, alignmentFloor ≤ alignment n i)
    (hgamma_floor : gamma ≤ weightFloor * alignmentFloor)
    (halignment :
      ∀ n i, alignment n i ≤ -centeredSource (w n) (source n) i) :
    ∀ n i,
  ...
  have hanti :
      w n i * alignment n i ≤
        w n i * (-centeredSource (w n) (source n) i) :=
    mul_le_mul_of_nonneg_left (halignment n i) hw_nonneg
  calc
    gamma ≤ weightFloor * alignmentFloor := hgamma_floor
    _ ≤ w n i * alignment n i := hweighted
...
theorem physicalHauptvermutungDistortion_source_local_response_of_centered_source_floor
  ...
  have hmul :
      rateFloor n * D i ≤
        (-(w n i * centeredSource (w n) (source n) i)) * D i :=
    mul_le_mul_of_nonneg_right (hsource_floor n i) hdist_nonneg
  calc
    rateFloor n *
        physicalHauptvermutungDistortion
          (countWindow n) (curvatureBias n) (spectralLocality n)
          (scale n) (edge n) (candidate n) i =
        rateFloor n * D i := by rfl
    _ ≤ (-(w n i * centeredSource (w n) (source n) i)) * D i := hmul
    _ =
        localLinearDescentContribution (w n) (source n)
          (physicalHauptvermutungDistortion
            (countWindow n) (curvatureBias n) (spectralLocality n)
...
theorem physicalHauptvermutungTotalDistortion_uniform_rate_of_source_local_response
  ...
  refine
    physicalHauptvermutungTotalDistortion_uniform_rate_floor_of_local_descent
      (rateFloor := fun _ => gamma)
      hcount hcurv hspectral htotal_eq ?_ hsource_local ?_
  · intro n
    rfl
  · intro n
    rw [hdescent_eq n,
...
theorem physicalHauptvermutungTotalDistortion_uniform_rate_of_centered_source_floor
    {ι : Type*} [Fintype ι]
    {w source countWindow curvatureBias spectralLocality : ℕ → ι → ℝ}
    {scale total descentRate : ℕ → ℝ}
    {edge : ℕ → ι → E4}
    ...
    (hsource_floor :
      ∀ n i,
        gamma ≤ -(w n i * centeredSource (w n) (source n) i))
    (hdescent_eq :
      ∀ n,
        descentRate n =
          -linearResponse (w n) (source n)
...
theorem physicalGrowthRepairRefinement_horizon_protection_and_total_tendsto_zero_of_centered_source_floor
    {ι : Type*} [Fintype ι]
    {w J source countWindow curvatureBias spectralLocality : ℕ → ι → ℝ}
    {scale c step descentRate remainder total rateFloor : ℕ → ℝ}
    {edge : ℕ → ι → E4}
  ...
  exact
    physicalGrowthRepairRefinement_horizon_protection_and_total_tendsto_zero_of_source_local_physical_uniform_rate_floor
      R gamma stepFloor hprod_pos hprod_le_two hcount hcurv hspectral
      htotal_eq hgamma_nonneg hgamma_le
      (physicalHauptvermutungDistortion_source_local_response_of_centered_source_floor
        hcount hcurv hspectral hsource_floor)
      hdescent_eq hstep_floor

theorem physicalGrowthRepairRefinement_horizon_protection_and_total_tendsto_zero_of_weighted_anti_alignment
    {ι : Type*} [Fintype ι]
    {w J source countWindow curvatureBias spectralLocality : ℕ → ι → ℝ}
    {scale c step descentRate remainder total rateFloor : ℕ → ℝ}
    {edge : ℕ → ι → E4}
  ...
  exact
    physicalGrowthRepairRefinement_horizon_protection_and_total_tendsto_zero_of_centered_source_floor
      ...
      (centeredSource_floor_of_weighted_anti_alignment
        hw_nonneg hweighted_floor halignment)
      hdescent_eq hstep_floor

theorem physicalGrowthRepairRefinement_horizon_protection_and_total_tendsto_zero_of_uniform_weighted_anti_alignment
    {ι : Type*} [Fintype ι]
    {w J source countWindow curvatureBias spectralLocality : ℕ → ι → ℝ}
    {scale c step descentRate remainder total rateFloor : ℕ → ℝ}
    {edge : ℕ → ι → E4}
    ...
    (R : PhysicalGrowthRepairRefinement w J source
      countWindow curvatureBias spectralLocality
      scale c step descentRate remainder total edge candidate)
    (gamma stepFloor weightFloor alignmentFloor : ℝ)
    (hprod_pos : 0 < stepFloor * gamma)
    (hprod_le_two : stepFloor * gamma ≤ 2)
    (hcount : ∀ n i, 0 ≤ countWindow n i)
    (hcurv : ∀ n i, 0 ≤ curvatureBias n i)
  ...
  exact
    physicalGrowthRepairRefinement_horizon_protection_and_total_tendsto_zero_of_weighted_anti_alignment
      R gamma stepFloor hprod_pos hprod_le_two hcount hcurv hspectral
      htotal_eq hgamma_nonneg hgamma_le
      (fun n i => le_trans hweight_nonneg (hweight_floor n i))
      (weighted_floor_of_uniform_weight_alignment hweight_nonneg
        halignment_nonneg hweight_floor halignment_floor hrate_floor)
      halignment hdescent_eq hstep_floor

theorem physicalGrowthRepairRefinement_horizon_protection_and_total_tendsto_zero_of_uniform_weight_alignment_floor
    {ι : Type*} [Fintype ι]
    {w J source countWindow curvatureBias spectralLocality : ℕ → ι → ℝ}
    {scale c step descentRate remainder total : ℕ → ℝ}
    {edge : ℕ → ι → E4}
    ...
    physicalGrowthRepairRefinement_horizon_protection_and_total_tendsto_zero_of_explicit_uniform_rate_floor
      ...
      (physicalHauptvermutungTotalDistortion_uniform_rate_of_centered_source_floor
        ...
        (centeredSource_gamma_floor_of_uniform_weighted_alignment
          hweight_nonneg halignment_nonneg hweight_floor halignment_floor
          hgamma_floor halignment)
        hdescent_eq)
      hstep_floor
...
theorem physicalGrowthRepairRefinement_horizon_protection_and_total_tendsto_zero_of_uniform_centered_source_product_floor
    {ι : Type*} [Fintype ι]
    {w J source countWindow curvatureBias spectralLocality : ℕ → ι → ℝ}
    {scale c step descentRate remainder total : ℕ → ℝ}
    {edge : ℕ → ι → E4}
    ...
    (hprod_pos : 0 < stepFloor * (weightFloor * sourceFloor))
  ...
  exact
    physicalGrowthRepairRefinement_horizon_protection_and_total_tendsto_zero_of_uniform_centered_source_floor
      R (weightFloor * sourceFloor) stepFloor weightFloor sourceFloor
      hprod_pos hprod_le_two
      hcount hcurv hspectral htotal_eq
      (mul_nonneg hweight_nonneg hsource_nonneg)
      hweight_nonneg hsource_nonneg hweight_floor hsource_floor
      le_rfl
      hdescent_eq hstep_floor
...
theorem physicalGrowthRepairRefinement_horizon_protection_and_total_tendsto_zero_of_stagewise_centered_source_unclipped_gain_floor
    {ι : Type*} [Fintype ι]
    {w J source countWindow curvatureBias spectralLocality : ℕ → ι → ℝ}
    {scale c step descentRate remainder total : ℕ → ℝ}
    {edge : ℕ → ι → E4}
        ...
        descentRate n =
          -linearResponse (w n) (source n)
            (physicalHauptvermutungDistortion
              ...
              (scale n) (edge n) (candidate n)))
    (hgain_lower :
      ∀ n, beta ≤ step n * (weightFloor n * sourceFloor n)) :
    (∀ n,
      linearResponse (w n) (source n) (finiteAreaChange (c n) (J n)) = 0 ∧
        quadraticResponse (w n) (source n)
          (finiteAreaChange (c n) (J n)) = 0) ∧
  ...
  refine
    physicalGrowthRepairRefinement_horizon_protection_and_total_tendsto_zero_of_stagewise_centered_source_clipped_gain_floor
      R beta hbeta_pos hcount hcurv hspectral htotal_eq
      hweight_pos hsource_pos hweight_floor hsource_floor hdescent_eq ?_
  intro n
  by_cases hle : weightFloor n * sourceFloor n ≤ (step n)⁻¹
  · have hmin :
        min (weightFloor n * sourceFloor n) (step n)⁻¹ =
          weightFloor n * sourceFloor n :=
      min_eq_left hle
    simpa [hmin] using hgain_lower n
  · have hmin :
        min (weightFloor n * sourceFloor n) (step n)⁻¹ =
          (step n)⁻¹ :=
      min_eq_right (le_of_lt (lt_of_not_ge hle))
    have hright : beta ≤ step n * (step n)⁻¹ := by
      have hcancel : step n * (step n)⁻¹ = 1 := by
        field_simp [ne_of_gt (R.step_pos n)]
...
theorem physicalGrowthRepairRefinement_horizon_protection_and_total_tendsto_zero_of_positive_stagewise_centered_source_component_floors
    {ι : Type*} [Fintype ι]
    {w J source countWindow curvatureBias spectralLocality : ℕ → ι → ℝ}
    {scale c step descentRate remainder total : ℕ → ℝ}
    {edge : ℕ → ι → E4}
    ...
    (∀ n,
      linearResponse (w n) (source n) (finiteAreaChange (c n) (J n)) = 0 ∧
        quadraticResponse (w n) (source n)
          (finiteAreaChange (c n) (J n)) = 0) ∧
      Tendsto total atTop (nhds 0) := by
  let beta : ℝ := min 1 (stepFloor * (weightBase * sourceBase))
  have hproduct_pos : 0 < stepFloor * (weightBase * sourceBase) :=
    mul_pos hstep_pos (mul_pos hweight_pos hsource_pos)
  have hbeta_pos : 0 < beta := by
    dsimp [beta]
    exact lt_min zero_lt_one hproduct_pos
  have hbeta_le_one : beta ≤ 1 := by
    dsimp [beta]
    exact min_le_left _ _
  have hgain_lower : beta ≤ stepFloor * (weightBase * sourceBase) := by
    dsimp [beta]
    exact min_le_right _ _
  exact
    physicalGrowthRepairRefinement_horizon_protection_and_total_tendsto_zero_of_stagewise_centered_source_component_gain_floor
      R beta stepFloor weightBase sourceBase
      hbeta_pos hbeta_le_one hweight_pos hsource_pos
      hcount hcurv hspectral htotal_eq
...
structure PhysicalHauptvermutungExactRecoveryCertificate
    {ι : Type*} [Fintype ι]
    (w J source countWindow curvatureBias spectralLocality : ℕ → ι → ℝ)
    (scale c step descentRate remainder total : ℕ → ℝ)
    (edge : ℕ → ι → E4)
    (candidate : ℕ → ι → Equiv.Perm Direction)
    (stepFloor weightBase sourceBase residualGap : ℝ) : Prop where
  convergence :
...
theorem PhysicalHauptvermutungRecoveredStage.candidate_transport
    {ι : Type*} [Fintype ι]
    {countWindow curvatureBias spectralLocality : ι → ℝ}
    {scale total : ℝ}
    {edge : ι → E4}
    ...
    (i : ι) :
    candidate i = fourState.perm (edge i) := by
  have hcandidate := R.candidate_eq_canonical
  simpa [canonicalCSpecBridgeCandidate] using congrFun hcandidate i

theorem PhysicalHauptvermutungRecoveredStage.physical_total_distortion_zero
    {ι : Type*} [Fintype ι]
    {countWindow curvatureBias spectralLocality : ι → ℝ}
    {scale total : ℝ}
    {edge : ι → E4}
    ...
    (R : PhysicalHauptvermutungRecoveredStage
      countWindow curvatureBias spectralLocality scale total edge candidate) :
    physicalHauptvermutungTotalDistortion
      countWindow curvatureBias spectralLocality scale edge candidate = 0 := by
  unfold physicalHauptvermutungTotalDistortion
  exact Finset.sum_eq_zero (fun i _ => R.local_distortion_zero i)
...
theorem PhysicalHauptvermutungRecoveredStage.base_distortion_zero
    {ι : Type*} [Fintype ι]
    {countWindow curvatureBias spectralLocality : ι → ℝ}
    {scale total : ℝ}
    {edge : ι → E4}
    ...
    (hspectral : ∀ i, 0 ≤ spectralLocality i) :
    physicalHauptvermutungBaseDistortion
      countWindow curvatureBias spectralLocality = 0 := by
  rcases R.residuals_zero hcount hcurvature hspectral with
    ⟨hcount_zero, hcurvature_zero, hspectral_zero⟩
  unfold physicalHauptvermutungBaseDistortion
  apply Finset.sum_eq_zero
  intro i _
  rw [hcount_zero i, hcurvature_zero i, hspectral_zero i]
  ring
...
theorem physicalHauptvermutungExactRecoveryCertificate_eventually_exact_zero
  ...
  exact
    physicalHauptvermutungConvergenceCertificate_eventually_exact_zero_of_residual_gap
      C.convergence C.residualGap_pos
      C.count_gap C.curvature_gap C.spectral_gap

theorem physicalHauptvermutungExactRecoveryCertificate_eventually_local_distortion_zero
    {ι : Type*} [Fintype ι]
    {w J source countWindow curvatureBias spectralLocality : ℕ → ι → ℝ}
    {scale c step descentRate remainder total : ℕ → ℝ}
    {edge : ℕ → ι → E4}
    ...
    ∀ᶠ n in atTop,
      ∀ i,
        physicalHauptvermutungDistortion
          (countWindow n) (curvatureBias n) (spectralLocality n)
          (scale n) (edge n) (candidate n) i = 0 := by
  have hexact :
      ∀ᶠ n in atTop,
        total n = 0 ∧
          (∀ i, countWindow n i = 0) ∧
            (∀ i, curvatureBias n i = 0) ∧
              (∀ i, spectralLocality n i = 0) ∧
                candidate n = canonicalCSpecBridgeCandidate (edge n) :=
    physicalHauptvermutungExactRecoveryCertificate_eventually_exact_zero C
  filter_upwards [hexact] with n hn
  rcases hn with ⟨_htotal, hcount, hcurvature, hspectral, hcandidate⟩
  intro i
  exact
    (physicalHauptvermutungDistortion_zero_iff
      (countWindow n) (curvatureBias n) (spectralLocality n)
      (scale n) (edge n) (candidate n) i
      (C.convergence.count_nonneg n i)
      (C.convergence.curvature_nonneg n i)
      (C.convergence.spectral_nonneg n i)).2
      ⟨hcount i, hcurvature i, hspectral i, by
        rw [hcandidate]
        rfl⟩

theorem physicalHauptvermutungExactRecoveryCertificate_eventually_full_operational_recovery
    {ι : Type*} [Fintype ι]
    {w J source countWindow curvatureBias spectralLocality : ℕ → ι → ℝ}
    {scale c step descentRate remainder total : ℕ → ℝ}
    {edge : ℕ → ι → E4}
      ...
      ∀ᶠ n in atTop,
        ∀ i,
          physicalHauptvermutungDistortion
            (countWindow n) (curvatureBias n) (spectralLocality n)
            (scale n) (edge n) (candidate n) i = 0 :=
    physicalHauptvermutungExactRecoveryCertificate_eventually_local_distortion_zero C
  have hbridge :
      ∀ᶠ n in atTop,
        cSpecBridgeTotalDistortion (scale n) (edge n) (candidate n) = 0 :=
  ...
  have horder :
    ...
    physicalHauptvermutungConvergenceCertificate_eventually_orderRecovered
      C.convergence
  filter_upwards [hexact, hlocal, hbridge, horder]
...
theorem physicalHauptvermutungExactRecoveryCertificate_exists_observable_zero_after
    {ι : Type*} [Fintype ι]
    {w J source countWindow curvatureBias spectralLocality : ℕ → ι → ℝ}
    {scale c step descentRate remainder total : ℕ → ℝ}
    {edge : ℕ → ι → E4}
      ...
      total n = 0 ∧
        ...
        (∀ i a b,
          Cov fourState (GPoint.atom (fourState.dst (edge n i)) b)
              (GPoint.bridge (edge n i) a) →
            b = candidate n i a) := by
  rcases physicalHauptvermutungExactRecoveryCertificate_exists_recovered_after
    C with ⟨N, hN⟩
  refine ⟨N, ?_⟩
  intro n hn
  let R : PhysicalHauptvermutungRecoveredStage
      (countWindow n) (curvatureBias n) (spectralLocality n)
      (scale n) (total n) (edge n) (candidate n) :=
    hN n hn
  have hresiduals :
  ...
  exact
    ⟨R.total_zero,
      R.physical_total_distortion_zero,
      R.base_distortion_zero
        (C.convergence.count_nonneg n)
        (C.convergence.curvature_nonneg n)
        (C.convergence.spectral_nonneg n),
      R.bridge_total_zero,
      R.candidate_eq_canonical,
      hresiduals.1,
      hresiduals.2.1,
      hresiduals.2.2,
      R.order_recovered⟩
...
#print axioms physicalGrowthRepairRefinement_horizon_protection_and_total_tendsto_zero_of_centered_source_floor
...
#print axioms PhysicalHauptvermutungRecoveredStage.candidate_transport
#print axioms PhysicalHauptvermutungRecoveredStage.physical_total_distortion_zero
#print axioms PhysicalHauptvermutungRecoveredStage.residuals_zero
#print axioms PhysicalHauptvermutungRecoveredStage.base_distortion_zero
...
#print axioms physicalHauptvermutungExactRecoveryCertificate_exists_observable_zero_after
