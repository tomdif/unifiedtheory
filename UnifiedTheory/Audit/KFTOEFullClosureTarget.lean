...
specialized to the Hayden-Preskill-native scrambling, decoupling, and
recovery-with-gap bridge.  This is narrower than the generic named Gate 6
target: the black-hole part now points at the precise HP analytic obligations
already used by the repo's Page-curve/recovery audit. -/
theorem microscopicTOEClosureTargetsWithFiniteGate1Gate4EstimatorArakiNamedGate5Gate6HaydenPreskillBridges_closed
    {η ι X Y chart : Type*} [Fintype η] [Fintype ι]
    [AddCommGroup Y] [Module ℝ Y] [Fintype chart] [Nonempty chart]
    {w J source countWindow curvatureBias spectralLocality : ℕ → ι → ℝ}
    {scale c step descentRate remainder total : ℕ → ℝ}
...
continuum lifts by named bridge records, and Gate 6's evaporation tail by the
Hayden-Preskill-native bridge. -/
theorem microscopicTOEClosureTargetsWithFiniteGate1NamedGate4ArakiHorizonNamedGate5Gate6HaydenPreskillBridges_closed
    {η ι X Y chart : Type*} [Fintype η] [Fintype ι]
    [AddCommGroup Y] [Module ℝ Y] [Fintype chart] [Nonempty chart]
    {w J source countWindow curvatureBias spectralLocality : ℕ → ι → ℝ}
    {scale c step descentRate remainder total : ℕ → ℝ}
  ...
  exact
    microscopicTOEClosureTargetsWithFiniteGate1Gate4EstimatorArakiNamedGate5Gate6HaydenPreskillBridges_closed
      ...
      hGate4Bridge.relativeEntropyAreaVariation
      hGate4Bridge.raychaudhuriAreaVariation
...
signed-fiber exponent table plus order-data coupling selection.  The remaining
tails are the named Gate 4 Araki-horizon bridge, named Gate 5 continuum
bridges, and the Hayden-Preskill-specialized Gate 6 bridge. -/
theorem microscopicTOEClosureTargetsWithGate1WitnessTableGate4ArakiHorizonNamedGate5Gate6HaydenPreskillBridges_closed
    {η ι X Y chart : Type*} [Fintype η] [Fintype ι]
    [AddCommGroup Y] [Module ℝ Y] [Fintype chart] [Nonempty chart]
    {w J source countWindow curvatureBias spectralLocality : ℕ → ι → ℝ}
    {scale c step descentRate remainder total : ℕ → ℝ}
