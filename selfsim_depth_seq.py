...
assert counts[min(NMAX, 7)] == [1, 2, 5, 16, 63, 318, 2045][min(NMAX, 7) - 1]
...
stems8 = [k for k in kk if nelem(k) == NMAX]
rows7, rhs7 = [], []
Tcol = {}
for C in stems8:
    m, rel = C
    ...
    for v in range(m):
        keep = [u for u in range(m) if u != v]
        gi = {u: i for i, u in enumerate(keep)}
        sub = canon_fast(NMAX - 1, tuple(sorted((gi[a], gi[b])
              for (a, b) in rel if a in gi and b in gi)))
        cnt[sub] = cnt.get(sub, 0) + 1
    Tcol[C] = {c: v / float(NMAX) for c, v in cnt.items()}
for c in sorted(k for k in kk if nelem(k) == NMAX - 1):
    rr = np.zeros(nvu); ri = np.zeros(nvu)
...
r_range(Aw, bw, "plain confined d%d" % NMAX)
print("one-step self-similar range (THE NUMBER):", flush=True)
r_range(Ass, bss, "self-similar d%d" % NMAX)
